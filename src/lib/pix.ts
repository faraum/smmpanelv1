...
export function generatePixCode(amount: number, description: string): string {
  ...
  const payload = [
    ...
    '5802BR',                                     // Country Code
    '5908Engajafy',                                 // Merchant Name
    '6009SAO PAULO',                              // Merchant City
    `62070503${description.slice(0, 3)}`,         // Additional Data
    '6304',                                       // CRC placeholder
  ].join('')

  return `00020126580014BR.GOV.BCB.PIX0136${pixKey}5204000053039865406${amountFormatted.replace('.', '').padStart(6, '0')}5802BR5908Engajafy5009Sao Paulo62070503${Date.now().toString().slice(-3)}6304ABCD`
}
